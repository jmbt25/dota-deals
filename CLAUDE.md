...
Python 3.12 (pinned `>=3.12,<3.13`) · async/await throughout · httpx · Pydantic v2 (+ pydantic-settings) · plain `sqlite3` with typed repositories · Typer (CLI) · structlog · tenacity · pytest + pytest-asyncio + respx · ruff · mypy --strict

## Non-negotiables

- **Type everything.** Every function signature, every Pydantic model field. `mypy --strict` must pass before any commit.
- **Handle errors intentionally.** No bare `except`. Catch specific exceptions (`httpx.TimeoutException`, `httpx.HTTPStatusError`, `pydantic.ValidationError`, `sqlite3.IntegrityError`, `json.JSONDecodeError`, `FileNotFoundError`, `ValueError`). Every caught exception either: re-raises with added context, routes to the dead-letter table, or is logged at WARNING+ with structured context (item_id, source, attempt).
