...
class StorageError(Exception):
    ...
    Repository functions translate D1's typed exceptions
    (:class:`D1QueryError` etc.) onto this hierarchy at the boundary,
    so callers don't have to know which transport produced the failure.
    """
...
class SchemaError(StorageError):
    ...
    For D1, schema lives in ``migrations/0001_initial.sql`` and is
    applied by ``wrangler d1 migrations apply`` — not by application
    code. The exception remains in the hierarchy for symmetry and so
    test helpers (e.g. :class:`tests._d1_fake.D1FakeClient`) that
    bootstrap a schema can raise it on failure.
    """


class IntegrityViolation(StorageError):
    """Raised when a write violates a non-uniqueness constraint (FK, CHECK).

    UNIQUE-on-PK collisions are not surfaced as integrity violations
    — they're the expected outcome of idempotent ``INSERT OR IGNORE``
    re-runs, which the affected repository functions handle silently.
    """
